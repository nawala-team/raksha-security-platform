...
//! Format: rkat_{org_id_prefix}_{random_32_chars}
...
pub fn generate_enrollment_token(req: GenerateTokenRequest) -> EnrollmentToken {
    ...
    let now = Utc::now();
    let expiry_hours = req.expiry_hours.unwrap_or(DEFAULT_EXPIRY_HOURS);
...
pub fn validate_token_format(token: &str) -> Result<(), TokenError> {
    ...
    if parts[2].len() != 32 { return Err(TokenError::InvalidRandomPart); }
    Ok(())
}
